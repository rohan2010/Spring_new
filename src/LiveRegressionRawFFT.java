...
public class LiveRegressionRawFFT extends PApplet implements AudioListener
{
	...
	public void keyReleased()
	{
	  ...
	  if (key=='t')
	  {
		  String rawFFT = "raw_fft_train.csv";
		  String completeFeatureSet = "train.csv";
		  saveCSV(rawFFT);
		  rawFFTToFeatures(rawFFT, completeFeatureSet);
		  REG = new WekaRegression(completeFeatureSet);
	  }
	...
	void rawFFTToFeatures(String rawFFTfilepath, String filepath)
	{
		String[] csv = loadStrings(rawFFTfilepath);
		String[] dataDump = new String[csv.length];
		int num_features = 0;
		...
		for (int i=1; i<csv.length; i++)
		{
			...
			for (int j=0; j<fields.length-1; j++) {
				fft.setBand(j, parseFloat(fields[j]));
			}
			
			ArrayList<Float> features = FeatureGenerator.featuresFromFFT(fft);
